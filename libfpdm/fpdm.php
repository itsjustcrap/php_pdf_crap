    ...
    class FPDM {
        ...
        function FPDM() {
            ...
            switch ($num_args) {
                ...
                case FPDM_STATIC:
                    if ($args[0] == '[_STATIC_]')
                        break; //static use, caller is anonymous function defined in _set_field_value 
...
//else this is the pdf_source then, fdf content is loaded using Load() function
        ...
        function Fix() {
            //---------------
            if (!$this->fields)
                $this->fields = array(); //Default: No field data
        ...
        function Output($dest = '', $name = '') {
            ...
            if ($this->support == "pdftk") {
                ...
                if ($this->fdf_source) { //FDF file provided
                    ...
                    if (is_file($this->fdf_source)) {
                        $fdf_file = $this->fdf_source;
                    }
                } else {
        ...
        function _set_field_value($line, $value) {
            ...
            if (preg_match($field_regexp, $CurLine)) {
                ...
                $CurLine = preg_replace_callback(
                        $field_regexp, create_function('$matches', $callback_code), $CurLine
                );
            } else {
                if ($verbose_set)
                    echo("<br>WARNING:" . htmlentities("Can not access to the value: $CurLine using regexp $field_regexp"));
            }
...
//					$this->Error("set_field_value failed as invalid valuetype $type for object $object_id");
        ...
        function set_field_tooltip($name, $value) {
            ...
            if (isset($this->value_entries["$name"])) {
                ...
                if ($field_tooltip_line) {
                    if ($verbose_set)
                        echo "<br>Change tooltip of the field $name at line $field_tooltip_line to value [$value]";
                    $offset_shift = $this->_set_field_value($field_tooltip_line, $value);
                }else {
                    if ($verbose_set)
                        echo "<br>Change toolpip value aborted, the field $name has no tooltip definition.";
                }
            } else
        ...
        function _hex2bin($hexString) {
            ...
            if ($hexLength % 2 != 0 || preg_match("/[^\da-fA-F]/", $hexString))
                return FALSE;
        ...
        function fix_xref_start() {
            ...
            if ($this->check_mode) { //Compared calculated value with position value read direct from file
                if ($xref_start_value_calculated != $xref_start_value_safe) {
                    if ($verbose_fix)
                        echo "<br>xrefstart's value must be $xref_start_value_safe calculated is $xref_start_value_calculated.Don't worry, FPDFM-merge will fix it for you.<br>";
                    ...
                    if ($verbose_fix)
                        echo "<br>xrefstart's value for the file is correct and vaults <b>$xref_start_value</b>";
                }
            }
            ...
            if ($xLen == $oLen) { //...to rectify xref entries 
                ...
                for ($i = 0; $i < $xLen; $i++) {
                    ...
                    if ($calculate_offset_value) {
                        ...
                        ;
                        if (!$this->safe_mode)
                            $offset_value = $offset_value_calculated;
                    }

                    if ($extract_offset_value_from_file) {
                        $offset_value_read = $this->read_offset_object_value($obj_id);
                        if ($this->safe_mode)
                            $offset_value = $offset_value_read;
                    }

                    if ($this->check_mode) {
                        if ($offset_value_calculated != $offset_value_read) {
                            if ($verbose_fix)
                                echo "<br>Offset for object $obj_id read is <b>$offset_value_read</b>, calculated $offset_value_calculated";
                            $offset_value = $offset_value_read; //overrides to fix bad values
                            if ($this->halt_mode)
                                $this->Error("<br>Offset for object $obj_id read is <b>$offset_value_read</b>, calculated $offset_value_calculated");
                        }else {
                            if ($verbose_fix)
                                echo "<br>Offset for object $obj_id is correct and vaults <b>$offset_value</b>";
                        }
                    }
        ...
        function parsePDFEntries(&$lines) {
            ...
            if ($this->verbose)
                $this->dumpContent("Starting to parse $CountLines entries", "PDF parse");
            ...
            while ($Counter < $CountLines) {
                ...
                if ($verbose_parsing)
                    $this->dumpContent($CurLine, "====Parsing Line($Counter)");
                if (!$xref_table) {
                    ...
                    if (preg_match("/^(\d+) (\d+) obj/", $CurLine, $match)) {
                        ...
                        $n++;
                        if ($verbose_parsing)
                            $this->dumpContent($CurLine, "====Opening object($obj) at line $Counter");
                        $object = array();
                        $object["values"] = array();
                        ...
                        if ($obj) {
                            ...
                            if (preg_match("/endobj/", $CurLine, $match)) {
                                if ($verbose_parsing)
                                    $this->dumpContent("", "====Closing object($obj) at line $Counter");
                                ...
                                if ($subtype == "Widget") {
                                    ...
                                    if ($name != '') {
                                        $lines["$name"] = $object;
                                        if ($verbose_parsing)
                                            $this->dumpContent("$type $subtype (obj id=$obj) is a text annotation of name '$name', saves it.");
                                    }//else
...
//									if(!array_key_exists("default",$values)) $this->Error("Cannot find default value (/DV) for field $name");
                                ...
                                if ($verbose_parsing)
                                    $this->dumpContent("Object $type $subtype (obj id=$obj) is not supported");
                                ...
                                if (preg_match("/\/Length\s*(\d+)/", $CurLine, $match)) {
                                    ...
                                    $stream["content"] = '';
                                    if ($verbose_parsing)
                                        $this->dumpContent($CurLine, "->Stream filter length definition(<font color=\"darkorange\">{$match[1]}</font>) for object($obj) at line $Counter");
                                }
                                ...
                                if (array_key_exists("length", $stream)) { //length is mandatory
                                    if (preg_match("/\b(stream|endstream)\b/", $CurLine, $match)) {
                                        ...
                                        if ($match[1] == "stream") {
                                            if ($verbose_parsing)
                                                $this->dumpContent($CurLine, "->Opening stream for object($obj) at line $Counter");
                                    ...
                                    if (($creationDate == '') && preg_match("/\/CreationDate\(([^\)]+)\)/", $CurLine, $values)) {
                                        ...
                                        if ($verbose_parsing)
                                            echo("Creation date read ($creationDate)");
                                    ...
                                    if (($type == '') || ($subtype == '') || ($name == "")) {
                                        ...
                                        if (($type == '') && $this->extract_pdf_definition_value("/Type", $CurLine, $match)) {
                                            ...
                                            if ($match[1] != 'Border') {
                                                $type = $match[1];
                                                if ($verbose_parsing)
                                                    echo("<br>Object's type is '<i>$type</i>'");
                                            }
                                        }
                                    ...
                                    if ($verbose_parsing)
                                        echo("WARNING: definition ignored");
...
//									} 
                        ...
                        if (preg_match("/\bxref\b/", $CurLine, $match)) {
                            ...
                            $xref_table = 1;
                            if ($verbose_parsing)
                                $this->dumpContent("->Starting xref table at line $Counter:[$CurLine]");
                            $lines['$_XREF_$'] = array();
                    ...
                    switch ($xref_table) {
                        case 2:
                            if (preg_match("/^(\d+) (\d+)/", $CurLine, $match)) {
                                ...
                                if ($verbose_parsing)
                                    $this->dumpContent("Xref table length is $refs_count");
                            }else
                            if ($verbose_parsing)
                                $this->dumpContent("WARNING: Xref table length ignored!");
                            break;
                        case 3:
                            //Should be 0000000000 65535 f 
                            if ($verbose_parsing)
                                $this->dumpContent("this is Xref table header, should be 0000000000 65535 f ");
                            break;
                        default:
                            //xref entries
                            if ($refs_count > 0) {
                                ...
                                if ($refs_count == 1) {//Last one , due to the shift, is the trailer
                                    ...
                                    if ($verbose_parsing)
                                        $this->dumpContent("Xref table entry for object $xref found.");
                                }
                                $refs_count--;
                                ...
                                if ($trailer_table == 1) { //should be <<
                                    ...
                                    if (preg_match("/\/Size (\d+)/", $CurLine, $match)) {
                                        ...
                                        if ($verbose_parsing)
                                            $this->dumpContent("Size read ($size_read) for pdf found.");
                                    }
                                    ...
                                    if (preg_match("/^\/ID\s*\[\s*<([\da-fA-F]+)/", $CurLine, $match)) {
                                        ...
                                        $id_def = true;
                                        if ($verbose_parsing)
                                            $this->dumpContent("ID chunk one ($oid) for pdf found.");
                                    ...
                                    if ($id_def) {//we are inside the ID definition
                                        if ($id_single_line_def || $id_multi_line_def) {
                                            ...
                                            if (preg_match("/([\da-fA-F]+)>.*$/", $CurLine, $match)) {
                                                ...
                                                if ($verbose_parsing)
                                                    $this->dumpContent("ID chunk two ($tid) for pdf found.");
                                                $id_def = false;
                                            } else
                                    ...
                                    if (preg_match("/^\/DocChecksum \/([\da-fA-F]+)/", $CurLine, $match)) {
                                        ...
                                        if ($verbose_parsing)
                                            $this->dumpContent("Checksum read ($checksum) for pdf found.");
                                    }
        ...
        function parseFDFContent() {
            ...
            if ($this->verbose)
                $this->dumpEntries($content, "FDF parse");
            ...
            if ($this->verbose)
                $this->dumpContent($fields, "FDF has no fields", false);

            if ($this->verbose)
                $this->dumpContent($fields, "FDF parsed", false);
